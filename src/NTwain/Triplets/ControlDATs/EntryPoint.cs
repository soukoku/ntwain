...
{
    public TWRC Get(TWIdentityWrapper app, out TW_ENTRYPOINT_DELEGATES entry)
    {
        entry = default;
        ...
        var rc = DoIt(app, MSG.GET, ref rawentry);
        if (rc == TWRC.SUCCESS)
        {
    ...
    static TWRC DoIt(TWIdentityWrapper app, MSG msg, ref TW_ENTRYPOINT entry)
    {
        var rc = TWRC.FAILURE;
        if (TWPlatform.IsWindows)
        {
            ...
            {
                rc = WinLegacyDSM.DSM_Entry(ref app.TW_IDENTITY_LEGACY, IntPtr.Zero, DG.CONTROL, DAT.ENTRYPOINT, msg, ref entry);
            }
            else
            {
                rc = WinNewDSM.DSM_Entry(ref app.TW_IDENTITY_LEGACY, IntPtr.Zero, DG.CONTROL, DAT.ENTRYPOINT, msg, ref entry);
            }
        }
        ...
        else if (TWPlatform.IsMacOSX)
        {
            ...
            {
                rc = OSXLegacyDSM.DSM_Entry(ref app.TW_IDENTITY_MACOSX, IntPtr.Zero, DG.CONTROL, DAT.ENTRYPOINT, msg, ref entry);
            }
            else
            {
                rc = OSXNewDSM.DSM_Entry(ref app.TW_IDENTITY_MACOSX, IntPtr.Zero, DG.CONTROL, DAT.ENTRYPOINT, msg, ref entry);
            }
        }
        return rc;
    }
